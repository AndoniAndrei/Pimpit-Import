...
export const parseCSVData = (text: string, requiredHeaders: string[]): Product[] => {
  ...
  for (let i = 0; i < table.length; i++) {
    ...
    if (requiredHeaders.every(reqHeader => lowercasedHeaders.includes(reqHeader))) {
      headerIndex = i;
  ...
  if (headerIndex === -1) {
    console.error("Could not find CSV header. Required headers:", requiredHeaders, "First 10 lines of file:", text.split(/\r?\n|\r/).slice(0, 10).join('\n'));
